...
function myReplace(str, before, after) {
	...
	if (before[0].match(/[A-Z]/)){
		let afterMod = after[0].toUpperCase() + after.slice(1);
		return str.replace(before, afterMod);
	}else if (after[0].match(/[A-Z]/) && before[0].match(/[a-z]/)){
		let afterMod = after[0].toLowerCase() + after.slice(1);
		return str.replace(before, afterMod);
	} else {
